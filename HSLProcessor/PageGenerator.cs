    ...
    internal static class PageGenerator
    {
        ...
        private static GenerateResult GenerateTitleDetail(DirectoryInfo template, DirectoryInfo directory)
        {
            try
            {
                ...
                foreach (var item in songList)
                {
                    ...
                    if (item.Artist.Name != "")
                        artistDetail =
                            string.Format("<li>Artist: <a href=\"../../artist/{2}/{1}.html\">{0}</a></li>\r\n",
                                item.Artist.Name, item.Artist.ArtistId, Utils.GetGuidPrefix(item.Artist.ArtistId));
                    var sourceDetail = "";
                    if (item.Source.Name != "")
                        sourceDetail =
                            string.Format("<li>Source: <a href=\"../../source/{2}/{1}.html\">{0}</a></li>\r\n",
                    ...
                    itemTemplate = itemTemplate.Replace("{{title_urlencoded-" + replacementCode + "}}",
                        WebUtility.HtmlEncode(item.Title));
                    itemTemplate = itemTemplate.Replace("{{generated-" + replacementCode + "}}",
                    ...
                    var writer =
                        new StreamWriter(
                            new FileStream(subbase.FullName + "/" + item.TitleId + ".html", FileMode.Create),
                            Encoding.UTF8);
        ...
        private static GenerateResult GenerateArtistDetail(DirectoryInfo template, DirectoryInfo directory)
        {
            try
            {
                ...
                foreach (var item in artistList)
                {
                    ...
                    var songListContent = list.Aggregate(
                        "<table id=\"content_table\"><tr class=\"row\"><th class=\"cell table_head\">Title</th><th class=\"cell table_head\">Source</th></tr>\r\n",
                        (current, titleItem) => current + string.Format(
                                                    "<tr class=\"row\"><td class=\"cell\"><a href=\"../../title/{4}/{2}.html\">{0}</a></td><td class=\"cell\"><a href=\"../../source/{5}/{3}.html\">{1}</a></td></tr>\r\n",
                                                    titleItem.Title, titleItem.Source.Name, titleItem.TitleId,
                                                    titleItem.Source.SourceId, Utils.GetGuidPrefix(titleItem.TitleId),
                                                    Utils.GetGuidPrefix(titleItem.Source.SourceId)));
                    songListContent += "</table>";

                    itemTemplate = itemTemplate.Replace("{{content-" + replacementCode + "}}", songListContent);
                    itemTemplate = itemTemplate.Replace("{{artist_urlencoded-" + replacementCode + "}}",
                    ...
                    var writer =
                        new StreamWriter(
                            new FileStream(subbase.FullName + "/" + item.ArtistId + ".html", FileMode.Create),
                            Encoding.UTF8);
                    writer.Write(itemTemplate);
                    writer.Flush();
                }
            ...
            try
            {
                ...
                foreach (var item in sourceList)
                {
                    ...
                    if (item.Series != null)
                        ...
                        {
                            var seriesLink = string.Format("<a href=\"../../series/{2}/{0}.html\">{1}</a>",
                                item.SeriesId, item.Series
                    ...
                    var songListContent = list.Aggregate(
                        "<table id=\"content_table\"><tr class=\"row\"><th class=\"cell table_head\">Title</th><th class=\"cell table_head\">Artist</th></tr>\r\n",
                        (current, titleItem) => current + string.Format(
                                                    "<tr class=\"row\"><td class=\"cell\"><a href=\"../../title/{4}/{2}.html\">{0}</a></td><td class=\"cell\"><a href=\"../../artist/{5}/{3}.html\">{1}</a></td></tr>\r\n",
                                                    titleItem.Title, titleItem.Artist.Name, titleItem.TitleId,
                                                    titleItem.Artist.ArtistId, Utils.GetGuidPrefix(titleItem.TitleId),
                                                    Utils.GetGuidPrefix(titleItem.Artist.ArtistId)));
                    songListContent += "</table>";

                    itemTemplate = itemTemplate.Replace("{{content-" + replacementCode + "}}", songListContent);
                    itemTemplate = itemTemplate.Replace("{{source_urlencoded-" + replacementCode + "}}",
                        WebUtility.HtmlEncode(item.Name));
                    itemTemplate = itemTemplate.Replace("{{generated-" + replacementCode + "}}",
                        DateTime.UtcNow.ToString("u"));
                    ...
                    var writer =
                        new StreamWriter(
                            new FileStream(subbase.FullName + "/" + item.SourceId + ".html", FileMode.Create),
                            Encoding.UTF8);
                    writer.Write(itemTemplate);
                    writer.Flush();
                }
        ...
        private static GenerateResult GenerateSeriesDetail(DirectoryInfo template, DirectoryInfo directory)
        {
            try
            {
                ...
                foreach (var item in seriesList)
                {
                    ...
                    var songListContent = list.Aggregate(
                        "<table id=\"content_table\"><tr class=\"row\"><th class=\"cell table_head\">Title</th><th class=\"cell table_head\">Artist</th><th class=\"cell table_head\">Source</th></tr>\r\n",
                        (current, titleItem) => current + string.Format(
                                                    "<tr class=\"row\"><td class=\"cell\"><a href=\"../../title/{6}/{2}.html\">{0}</a></td><td class=\"cell\"><a href=\"../../artist/{7}/{3}.html\">{1}</a></td><td class=\"cell\"><a href=\"../../source/{8}/{5}.html\">{4}</a></td></tr>\r\n",
                                                    titleItem.Title, titleItem.Artist.Name, titleItem.TitleId,
                                                    titleItem.Artist.ArtistId, titleItem.Source.Name,
                                                    titleItem.SourceId, Utils.GetGuidPrefix(titleItem.TitleId),
                                                    Utils.GetGuidPrefix(titleItem.Artist.ArtistId),
                                                    Utils.GetGuidPrefix(titleItem.Source.SourceId)));
                    songListContent += "</table>";

                    itemTemplate = itemTemplate.Replace("{{content-" + replacementCode + "}}", songListContent);
                    itemTemplate = itemTemplate.Replace("{{series_urlencoded-" + replacementCode + "}}",
                        WebUtility.HtmlEncode(item.Name));
                    itemTemplate = itemTemplate.Replace("{{generated-" + replacementCode + "}}",
                        DateTime.UtcNow.ToString("u"));
                    ...
                    var writer =
                        new StreamWriter(new FileStream(subbase + "/" + item.SeriesId + ".html", FileMode.Create),
                            Encoding.UTF8);
                    writer.Write(itemTemplate);
                    writer.Flush();
                }
        ...
        private static GenerateResult GenerateListing(DirectoryInfo template, DirectoryInfo directory)
        {
            try
            {
                ...
                songsTemplate = songsTemplate.Replace("{{title_count-" + replacementCode + "}}",
                    context.Songs.Count().ToString());
                songsTemplate = songsTemplate.Replace("{{artist_count-" + replacementCode + "}}",
                    context.Artists.Count().ToString());
                songsTemplate = songsTemplate.Replace("{{source_count-" + replacementCode + "}}",
                    context.Sources.Count().ToString());
                songsTemplate = songsTemplate.Replace("{{series_count-" + replacementCode + "}}",
                    context.Series.Count().ToString());

                var songListOutput =
                    "<table id=\"content_table\"><tr class=\"row\"><th class=\"cell table_head\">Title</th><th class=\"cell table_head\">Artist</th><th class=\"cell table_head\">Source</th></tr>\r\n{{content-" +
                    replacementCode + "}}</table>";
                var songListContent = songList.Aggregate("",
                    (current, item) => current + string.Format(
                                           "<tr class=\"row\"><td class=\"cell\"><a href=\"title/{6}/{3}.html\">{0}</a></td><td class=\"cell\"><a href=\"artist/{7}/{4}.html\">{1}</a></td><td class=\"cell\"><a href=\"source/{8}/{5}.html\">{2}</a></td></tr>\r\n",
                                           item.Title, item.Artist.Name, item.Source.Name, item.TitleId,
                                           item.Artist.ArtistId, item.Source.SourceId,
                                           Utils.GetGuidPrefix(item.TitleId), Utils.GetGuidPrefix(item.ArtistId),
                                           Utils.GetGuidPrefix(item.SourceId)));
                ...
                songsTemplate = songsTemplate.Replace("{{generated-" + replacementCode + "}}",
                    DateTime.UtcNow.ToString("u"));

                var writer = new StreamWriter(new FileStream(directory.FullName + "/songs.html", FileMode.Create),
                    Encoding.UTF8);
        ...
        public static GenerateResult ExportSitemap(FileInfo file, string urlbase)
        {
            try
            {
                ...
                foreach (var item in context.Songs)
                {
                    ...
                    songitem.Add(new XElement(ns + "loc",
                        urlbase + "title/" + Utils.GetGuidPrefix(item.TitleId) + "/" + item.TitleId + ".html"));
                    xl.Add(songitem);
                }
                ...
                foreach (var item in context.Artists)
                {
                    var songitem = new XElement(ns + "url");
                    songitem.Add(new XElement(ns + "loc",
                        urlbase + "artist/" + Utils.GetGuidPrefix(item.ArtistId) + "/" + item.ArtistId + ".html"));
                    xl.Add(songitem);
                }
                ...
                foreach (var item in context.Sources)
                {
                    var songitem = new XElement(ns + "url");
                    songitem.Add(new XElement(ns + "loc",
                        urlbase + "source/" + Utils.GetGuidPrefix(item.SourceId) + "/" + item.SourceId + ".html"));
                    xl.Add(songitem);
                }
                ...
                foreach (var item in context.Series)
                {
                    var songitem = new XElement(ns + "url");
                    songitem.Add(new XElement(ns + "loc",
                        urlbase + "series/" + Utils.GetGuidPrefix(item.SeriesId) + "/" + item.SeriesId + ".html"));
                    xl.Add(songitem);
                }
